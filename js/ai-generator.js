...
const AIGenerator = (() => {
    ...
    function getApiKey() {
        ...
        if (input && input.value.trim() !== '') {
            localStorage.setItem(`neurocode-${provider}-key`, input.value.trim());
            return input.value.trim();
        }
        return localStorage.getItem(`neurocode-${provider}-key`);
    }

    // Exported function to set keys manually from backend
    function setApiKeyFromBackend(prov, key) {
        localStorage.setItem(`neurocode-${prov}-key`, key);
        if (provider === prov) {
            const input = document.getElementById('ai-api-key');
    ...
    async function fetchAvailableModels() {
        if (provider === 'ollama') {
            ...
            return ['meta/llama3-70b-instruct', 'mistralai/mixtral-8x22b-instruct-v0.1', 'nvidia/nemotron-4-340b-instruct', 'google/gemma-7b'];
        } else {
...
3. Return ONLY valid JSON. Do not use markdown code blocks. Just the raw JSON.`;
    ...
    async function generateViaGemini(codeStr, inputStr) {
        ...
        if (data.error) {
            if (data.error.code === 400 || (data.error.message && data.error.message.includes('API key'))) {
                localStorage.removeItem(`neurocode-${provider}-key`);
    ...
    async function generateViaNvidia(codeStr, inputStr) {
        const apiKey = getApiKey();
        if (!apiKey) {
            throw new Error('NVIDIA API Key is required. Enter it in the toolbar.');
        }
    ...
    return {
        ...
        getApiKey, setApiKeyFromBackend
    };
})();
